...
public class Advent22b_v2 {

    private static final int EXTRA = 100;

    public static void main(String[] args) throws IOException {
        String svar = new Advent22b_v2().calc(Online.get());
...
//            System.out.println();
        ...
        Queue<Pair<Integer, Triple<Integer, Integer, Integer>>> q = new PriorityQueue<>();
        q.add(Pair.of(0, Triple.of(TORCH, 0, 0)));
        state[TORCH][0][0] = 0;
        int apa = 0;
        while (!q.isEmpty()) {
            apa++;
            Pair<Integer, Triple<Integer, Integer, Integer>> remove = q.remove();
            Triple<Integer, Integer, Integer> now = remove.getRight();
            ...
            for (int newTool = 0; newTool < 3; newTool++) {
                if (tool != newTool && newTool != G[y][x]) {
                    if (state[newTool][y][x] > state[tool][y][x] + 7) {
                        ...
                        q.add(Pair.of(state[newTool][y][x], Triple.of(newTool, x, y)));
                    }
                }
                ...
                if (!(nyy < 0 || nyy >= targety + EXTRA || nyx < 0 || nyx >= targetx + EXTRA)) {
                    if (tool != G[nyy][nyx]) {
                        if (state[tool][nyy][nyx] > state[tool][y][x] + 1) {
                            ...
                            q.add(Pair.of(state[tool][nyy][nyx], Triple.of(tool, nyx, nyy)));
                        }
                    }
                }
            }
