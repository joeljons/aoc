...
public class Advent19b {
    ...
    public static void main(String[] args) throws IOException {
        var svar = new Advent19b().calc(Online.get());
    ...
    private int calc(List<String> strs) {
        ...
        aligned[0] = true;
        scanners.get(0).scannerPos = new Point(0, 0, 0);
        boolean allAligned;
        do {
        ...
        for (int i = 0; i < nScan; i++) {
            Point pi = scanners.get(i).scannerPos;
            for (int j = i + 1; j < nScan; j++) {
                Point pj = scanners.get(j).scannerPos;
                int now = Math.abs(pj.g[0] - pi.g[0])
                        + Math.abs(pj.g[1] - pi.g[1])
                        + Math.abs(pj.g[2] - pi.g[2]);
