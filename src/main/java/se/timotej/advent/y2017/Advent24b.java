...
public class Advent24b {
    public static void main(String[] args) throws IOException {
        ...
        new Advent24b().calc(Online.get(24));
    }
    ...
    private void calc(List<String> strs) {
        ...
        int svar = rek(g, best, taken, 0)%10000;
        System.out.println("svar = " + svar);
        Online.submit(24, 2, svar);
    }
        ...
        for (Pair<Integer, Integer> integerIntegerPair : g) {
            if (integerIntegerPair.getKey() == now && !taken[i]) {
                ...
                int svar = integerIntegerPair.getKey() + integerIntegerPair.getValue() + rek(g, best, taken,
                        integerIntegerPair.getValue())+10000;
                if (svar > bestSvar) {
                    bestSvar = svar;
                }
                taken[i] = false;
            ...
            if (integerIntegerPair.getValue() == now && !taken[i]) {
                taken[i] = true;
                int svar = integerIntegerPair.getKey() + integerIntegerPair.getValue() + rek(g, best, taken,
                        integerIntegerPair.getKey())+10000;
                if (svar > bestSvar) {
                    bestSvar = svar;
                }
                taken[i] = false;
