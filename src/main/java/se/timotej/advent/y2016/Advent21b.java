...
public class Advent21b {
    ...
    private String calc(List<String> strs) {
        Character[] start = ArrayUtils.toObject(pwd);
        do {
            pwd = ArrayUtils.toPrimitive(start);
            for (String str : strs) {
        ...
        } while (!"fbgdceah".equals(new String(pwd)) && Util.nextPermutation(start));
        return new String(ArrayUtils.toPrimitive(start));
    }
